...
public class Controller {
            ...
            final Animation animation = new Transition() {
                {
                    ...
                    setOnFinished((e) -> {
                        ...
                        processText.appendText("[" + order[orderPosition] + "]\n" +
                                                order[orderPosition] + " have a " + numRoll + "\n");
                        if(!checkPlaneOnMap() && numRoll != 6){
                            processText.appendText("you have to roll a 6 to start\n");
                            numRoll = 0;
                            nextPlayer();
                            processText.appendText(order[orderPosition] + " turn next.\n");
                            rollDone = false;
                        }
                        if(numRoll == 6){
    ...
    public void goBlue(MouseEvent a){
        if(!rollDone){
            processText.appendText("[" + order[orderPosition] + "]\n" +
                                    order[orderPosition] + " please roll the dice.\n");
        }
        else{
        ...
        if(totalMap.size() == 0){
            ...
            for (Node nd: totalMap.keySet()){
                if(totalMap.get(nd) == route[s] && !nd.getId().equals(n.getId())){
                    ...
                    processText.appendText("You have crashed a plane. Great!\n");
    ...
    public boolean checkPlaneOnMap(){
        if(nowPos[orderPosition] != -1){
            ...
            //processText.setText(record);
            processText.appendText("[" + order[orderPosition] + "]\n" +
                                    "choose your " + order[orderPosition] + " plane on field\n");
            return true;
        }
        else{
            ...
            //processText.setText(record);
            processText.appendText("[" + order[orderPosition] + "]\n" +
                                    "no " + order[orderPosition] + " plane on the field\n");
            return false;
        }
    }
